...
class TkinterMapView(tkinter.Frame):
    def __init__(self, *args,
        ...
        self.button_zoom_out = CanvasButton(self, (20, 80), text="-", command=self.button_zoom_out)
        self.button_reset = CanvasButton(self, (20, 140), text="🏠", command=self.button_go_home)
        ...
        self.set_zoom(17)
        self.my_home = [52.516268, 13.377695]
        self.set_position(self.my_home[0],self.my_home[1])  # Brandenburger Tor, Berlin
    ...
    def mouse_move(self, event):
        ...
        if (self.last_mouse_down_position == None):
            mouse_move_x = 0
            mouse_move_y = 0
        else:
    ...
    def mouse_release(self, event):
        ...
        if self.mouse_click_position == (event.x, event.y):
            ...
            # delete the current mouse positions, because we don't care anymore
            self.forget_mouse_position()
            self.after(1, self.fading_move)

    def forget_mouse_position(self):
        # Called when we need to simply forget the mouse position (for use on touchscreen)
    ...
    def button_go_home(self):
        #go to saved home location
        self.set_position(self.my_home[0], self.my_home[1])