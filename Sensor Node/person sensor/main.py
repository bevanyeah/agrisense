...
from FaceDetect import PersonSensor
...
facedetect = PersonSensor(bus=bus, scl=scl, sda=sda, freq=freq)
...
while True:
   ...
   # try:
    face_detect = facedetect.look_for_face()
   ...
   # except:  
    ...
    if face_detect > 30:
        ...
        LED.value(1)
        old_face = face_detect
        
        # To counter any mis reads, let's wait for .5 sec and read again
        sleep_ms(500)
        ...
        if face_detect > 30 and old_face != face_detect:
            #important, stay awake!
            ...
            update_gps = ticks_ms()
            read_nmea()
            sx.send(build_payload(latlong_string, epoch_string, 1, f"{int(face_detect):02d}"))
            print("sent event")
