...
class PiicoDev_LIS3DH(object):
    def __init__(self, bus=None, freq=None, sda=None, scl=None, address=_I2C_ADDRESS, asw=None, range=2, rate=400):
        ...
        if asw not in [0, 1]:
            self.address = address  # default address used if asw not provided OR invalid
        else:
            self.address = [_I2C_ADDRESS, _I2C_ADDRESS - 1][asw]
    ...
    def range(self, r):
        ...
        try:
            ...
            raise ValueError("range must be one of 2, 4, 8, or 16")
    ...
    def rate(self, r):
        ...
        valid_rates = {0: 0b0000, 1: 0b0001, 10: 0b0010, 25: 0b0011, 50: 0b0100, 100: 0b0101, 200: 0b0110,
                       400: 0b0111}  # key:value -> rate[Hz] : binary code for register
        try:
            rr = valid_rates[r]
        except(KeyError):
            raise ValueError("rate must be one of 0, 1, 10, 25, 50, 100, 200, or 400")
